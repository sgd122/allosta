...
본 플랫폼은 세 가지 알림 이벤트를 처리해야 한다.

1. **CONFIRMATION**: 예약 생성 직후 즉시 발송
2. **REMINDER**: 예약 시각 전 일정 리드타임에 스케줄 잡으로 발송
3. **SLOT_OPENED**: 해당 상담사 예약 취소 시 waitlist FIFO 첫 번째 대기자에게 발송
...
| **알림 컴포넌트 전체 생략** | 구현 시간 절약 | "알림 설계 없음"으로 평가됨; REMINDER·SLOT_OPENED 이벤트가 AC5·AC10에서 미증명 | 설계 역량 입증 동인에 반함. 인터페이스 설계 비용은 stub 파일 2개이며, 얻는 신호는 크다. |
