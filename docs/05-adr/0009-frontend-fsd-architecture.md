...
2. **경계 부재**: 어떤 코드가 어떤 도메인에 속하는지, 무엇이 무엇을 import 해도 되는지에 대한 규칙이 없다.
...
- **`src/` 하위 레이어 + 얇은 라우팅**: 모든 FSD 레이어를 `src/` 아래 둔다. Next 라우팅은 `src/app/`(Next가 공식 지원)으로 옮겨 **라우팅 전용 얇은 레이어**로 유지하고, 각 `src/app/**/page.tsx`는 `src/views/<route>`를 재노출(`export { default } from '@/views/<route>'`)만 한다. `src/app/`가 Next 라우터 겸 FSD `app` 레이어(루트 레이아웃·providers·전역 스타일·API Route Handler) 역할을 겸한다.
