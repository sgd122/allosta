...
본 과제는 프론트엔드(React/TypeScript)와 백엔드(NestJS/TypeScript)를 모두 포함한다.
이 두 컴포넌트를 하나의 레포지토리에 둘 것인지, 분리된 레포지토리로 관리할 것인지 결정해야 한다.

핵심 제약: **재현성(외부 계정 0개, 단일 명령 기동).** 평가자가 레포를 받아 최소한의 명령으로
golden path를 끝까지 재현할 수 있어야 한다. 레포 구조가 이 마찰을 높이면 안 된다.
...
- 실 운영 전환 시 FE·BE를 독립 배포해야 한다면 레포 분리 또는 `git subtree` 분리가 필요할 수 있다. 평가 과제의 단일 레포 구조는 이 전환을 막지 않는다(디렉토리 경계가 명확히 분리되어 있으므로).
