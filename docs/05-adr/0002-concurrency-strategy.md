...
| **비관적 락 (`SELECT ... FOR UPDATE`)** | 충돌 전 차단, 결과 예측 명확 | 락 경합·타임아웃 관리 필요; 트랜잭션 길어짐; 데드락 리스크; 단건 슬롯 예약에서 추가 복잡도 대비 이득 없음 | 2주 솔로 과제에서 락 관리 오버헤드 과함. 슬롯 분할 예약·waitlist 자동매칭으로 확장 시 재검토 항목으로 설계 문서에 기록 |
...
- waitlist 자동매칭(R8, Phase 2) 구현 시 공석 탐지 + Booking INSERT 원자 묶음 트랜잭션 설계 필요.
