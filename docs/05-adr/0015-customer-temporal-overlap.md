...
1. **비정규화된 슬롯 윈도우(`Booking.slotStartAt` / `slotEndAt`)**: 시간 정보는 `AvailabilitySlot`에 있으나, EXCLUDE 제약은 단일 `Booking` 행에 대해 범위를 만들 수 있어야 한다. 따라서 예약된 슬롯의 `[startAt, endAt)` 윈도우를 `Booking`에 비정규화해 복제한다. 이 컬럼은 `create()` 시점에 **write-once**로 설정된다 — 예약은 슬롯을 재조정(reschedule)하지 않기 때문이다.
2. **`btree_gist` 확장**: 스칼라 동치(`customerId WITH =`)와 범위 겹침(`tsrange WITH &&`)을 하나의 GiST EXCLUDE 제약에 함께 넣으려면 `btree_gist`가 필요하다.
3. **ACTIVE 상태에 대한 부분 제약**: `WHERE status IN ('PENDING','CONFIRMED')` — `booking_slot_active_unique`와 정확히 동일한 부분 조건. CANCELLED/COMPLETED/NO_SHOW 예약은 제약에서 빠지므로, 취소 후 동일 시간대 재예약이 자연스럽게 허용된다.
...
- 23P01을 `409`로 매핑하는 코드가 서비스 레이어에 필요하다(Prisma가 전용 code를 제공하지 않으므로 message 기반 매칭).
