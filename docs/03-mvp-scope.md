...
**인스코프**: `/results` 추가형 해석 Q&A(본인 데이터 그라운딩), LLM(Ollama 재사용) + 결정론 템플릿 폴백, 결정론 out-of-scope 가드(질문 게이트 + 답변 후처리), 예/아니오 피드백, 어드민 이탈 지표(도움됨 응답률 + 행동적 이탈, 전역 스코프).

**아웃오브스코프(non-goals)**: 진단·치료·처방·복용량·식단·영양제 조언, 유료 상담 대체, 클라우드 LLM/외부 키, 크로스세션 장기 메모리, 실시간 스트리밍 토큰 UI, 상담사 대면 기능 변경.
