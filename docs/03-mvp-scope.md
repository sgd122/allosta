...
- **깊이 철학**: 모든 것을 똑같이 깊게 만들지 않는다. 핵심 설계(예약·기록·집계)는 REAL로 끝까지, 외부 의존 컴포넌트는 SEED/SIMULATED로 경계를 그어 핵심에 예산을 집중한다.
- **REAL 구현**: Booking, Consultation Log/CRM(+챌린지 등록+CallLog), Analytics, Family Linking(단순형), 상담 준비·생산성 자동화.
- **경계 설계**: Test Results = SEED + READ-ONLY(`UploadPipeline` 인터페이스), Notifications = SIMULATED(`NotificationChannel` 어댑터).
- **헤드라인 Non-Goals**: 실 결제/커머스, 실 SMS/카카오 발송, 검사결과 업로드/파싱, 가족 위임 OAuth, PDF/정산, reschedule, 비동기 상담, 고객 대기열.
- **Non-Goal 원칙**: "안 했다"가 아니라 "근거 있게 미뤘거나 인터페이스로 경계를 설계했다"로 서술한다.
- **AI Q&A 범위(ADR 0018)**: `/results` 해석 Q&A는 인스코프, 진단·처방·유료 상담 대체·클라우드 LLM은 아웃오브스코프.

---

## 1. 컴포넌트별 구현 깊이 표

아래 표는 7개 컴포넌트의 구현 깊이(REAL/SEED/SIMULATED)와 그 선택 근거를 정리한다.
...
- 전환(구매)은 상담사가 상담 직후 outcome 필드(`EXPLAINED | GUIDED | PURCHASED`)에 수동 입력하는 방식으로 대체한다.
- 수동 집계 자체가 R2(기록 스키마 부재)가 만든 페인이었다. 따라서 구조화 입력 + 자동 집계가 결제 연동 없이도 전환 가치를 증명한다.
- 실 결제 시스템은 외부 PG 계약·웹훅·환불 처리 등 독립적인 설계 난제를 수반하며, 2주 평가과제의 범위를 벗어난다.

### 2.2 실제 SMS/카카오 알림 발송

**제외 근거**:

- 재현성 동인(외부 계정 0개)과 정면으로 충돌하는 항목이다. 실제 SMS/카카오 연동은 계정·API 키·수신 번호를 요구해 평가자가 그대로 실행하는 것이 불가능하다.
- 대신 "확장 가능한 설계"임을 코드 수준에서 드러낸다:
  - `NotificationChannel` 인터페이스를 정의한다.
  - `ConsoleChannel`/`InAppChannel`은 실동작으로 제공한다.
  - `EmailChannel`/`SmsChannel`은 어댑터 stub으로 제공한다(주석으로 실 구현 경로 명시).
- 실 발송 어댑터 구현은 Phase 2.

### 2.3 검사결과 업로드/파싱 파이프라인

**제외 근거**:

- 검사 결과 업로드는 독립적인 서브시스템이다. 결과지 포맷(PDF, HL7, CSV 등) 파싱, 파일 저장소, 비동기 처리 파이프라인을 포함한다.
- MVP에서 TestResult를 seed JSON으로 제공한다. 이로써 Booking·Analytics와의 연결(TestResult 기반 subject 파생, 지표 귀속)은 완전히 동작한다.
- 경계 설계: `UploadPipeline` 인터페이스를 `test-result/upload-pipeline.interface.ts`에 정의하고 설계 주석으로 확장 경로를 명시한다.
- 실 구현은 Phase 2.
...
- 가족 연결의 핵심 가치는 보호자가 가족의 검사 결과를 상담 대상으로 지정할 수 있다는 것이다. 이는 `FamilyLink` 초대 코드 기반 연결로 충분히 증명된다.
- 복잡한 위임 권한 모델은 OAuth 위임, 접근 동의 관리, 별도 인증 플로우를 요구하는 독립 과제다.
- Phase 2에서 확장 경로를 설계한다.

### 2.5 결과지 PDF 생성, 결제 수단 관리, 정산

**제외 근거**:

- PDF 생성은 결과지 렌더링·서명·보관 등 별도 서브시스템이다. 정산은 결제 연동(2.1 참조)이 선행된다.
- MVP의 가치 증명(예약→확정→상담기록→전환집계)과 독립적이며, 구현 시 핵심 설계에 쓸 예산을 침식한다.
- Phase 2 설계 경계로 기록한다.
...
- reschedule은 R6(no-show 근본원인)의 대응이다. 그러나 MVP는 리마인더 알림(시뮬레이션)으로 R6에 부분 대응한다.
- reschedule 자체는 복합 플로우를 요구한다: 예약 상태 전이(CONFIRMED→RESCHEDULED→RE-CONFIRMED), 기존 슬롯 반환, 새 슬롯 동시성 재보장. 2주 예산에서 독립적으로 올바르게 구현하기 어렵다.
- "완성된 플로우"보다 "설계된 경계"가 더 강한 신호다. 따라서 상태 전이 다이어그램과 API 설계를 `docs/04-system-design.md`에 명시하고 구현은 Phase 2로 배치한다.

### 2.7 비동기 상담 대안 (결과지 FAQ·텍스트 Q&A)

**제외 근거**:

- R7(동기 상담만 존재)에 대한 더 깊은 대응이다.
- 비동기 상담은 별도 채널(채팅, 게시판, 전문가 답변 큐)을 필요로 한다. 이는 현재 도메인 모델(Booking 기반 동기 예약)과 근본적으로 다른 아키텍처를 요구한다.
- MVP 범위를 동기 예약 패러다임으로 고정하고, 비동기 채널 확장 경로를 Phase 2 설계 항목으로 기록한다.

### 2.8 고객 대기열(waitlist/queue) — 공석 통지·FIFO 승격·자동매칭

**제외 근거**:

- R4(만석 이탈)의 **1차 대안은 셀프서비스 가용 캘린더**다. 만석 시 고객이 다른 일자·상담사의 빈 슬롯을 직접 보고 스스로 예약하면(통합 캘린더 + `availableCount`, 컴포넌트 1) 원래의 "전화 술래잡기 → 조용한 이탈" 마찰이 이미 제거된다.
- limited counselor pool에서는 *가용 탐색*이 대기열보다 더 높은 레버리지의 1차 해법이다.
- 고객 대기열은 그 위의 잔여 통지 수요(R8)다(원하는 슬롯이 열리면 통지받고 FIFO로 승격받는 큐). 별도 복잡도를 요구한다: TTL/FIFO/promotion 상태기계 + 공석 통지 + 오퍼 만료/전환 추적.
- 이 복잡도 대비 MVP 가치가 낮으므로 **의식적으로 Phase 2 Non-Goal로 미룬다**.
- 자동매칭(상담사·시간 최적 추천)은 그보다 더 위의 추천 알고리즘 과제다.
- "가용 캘린더로 1차 이탈을 먼저 막고 대기열을 근거 있게 미룬 것"이 단순 미구현과 다른 점이다.

---

## 3. 근본원인별 MVP 배치 근거

아래 표는 9개 근본원인(R1~R9)을 MVP 배치(구현 / Phase 2 설계)와 그 근거로 정리한다.
...
## 고객용 AI Q&A 범위 (ADR 0018)

**인스코프**:

- `/results` 추가형 해석 Q&A(본인 데이터 그라운딩)
- LLM(Ollama 재사용) + 결정론 템플릿 폴백
- 결정론 out-of-scope 가드(질문 게이트 + 답변 후처리)
- 예/아니오 피드백
- 어드민 이탈 지표(도움됨 응답률 + 행동적 이탈, 전역 스코프)

**아웃오브스코프(non-goals)**:

- 진단·치료·처방·복용량·식단·영양제 조언
- 유료 상담 대체
- 클라우드 LLM/외부 키
- 크로스세션 장기 메모리
- 실시간 스트리밍 토큰 UI
