...
> 30초 오리엔테이션 — 이 문서의 핵심 설계 결정 6가지.

- **동시성**: DB unique constraint + insert-first로 중복 예약을 막는다. check-then-insert를 쓰지 않아 TOCTOU 창이 구조적으로 없다. PENDING 상태도 슬롯을 선점한다 ([§5](#5-동시성-설계)).
- **subject**: subject는 항상 `CUSTOMER`다. 가족 검사결과 접근은 대칭형 `FamilyLink`(Customer↔Customer)로 처리하며, 다형 참조는 제거됐다 ([§4](#4-subject-파생-및-가족-연결-설계)).
- **가용성**: 예약 가능 여부는 별도 컬럼이 아니라 파생값이다. `isOpen`은 운영 플래그일 뿐이고, 가용성은 쿼리로 계산해 이중 진실원을 없앤다 ([§6](#6-가용성-파생값-설계)).
- **권한 2층**: 역할 RBAC 층(NestJS Guard)과 자원 소유권 층(서비스 레이어)을 분리한다. 한 층이 다른 층을 대체하지 않는다 ([§8](#8-권한-2층-설계)).
- **프론트엔드**: Next.js App Router 기반 Feature-Sliced Design(FSD). JWT는 httpOnly 쿠키에 격리하고 Middleware가 서버측 역할 보호를 담당한다 ([§9](#9-프론트엔드-아키텍처)).
- **고객 시간대 중복 방지**: GiST `EXCLUDE` 제약(`booking_customer_no_overlap`)으로 한 고객이 겹치는 시간대에 복수 ACTIVE 예약을 갖는 것을 DB 레벨에서 차단한다 ([§5.x](#5x-고객-시간대-중복-방지-gist-exclude-adr-0015)).
...
**한 줄 요약**: 수동 전화 예약·상담 기록을 셀프서비스로 대체하고, 단일 골든 패스를 end-to-end로 증명하는 플랫폼이다.

- 본 플랫폼은 수동 전화 예약·상담 기록 프로세스를 셀프서비스로 대체한다.
- 세 역할(고객·상담사·관리자)이 각자의 뷰를 가진다.
- 1차 목표는 단일 골든 패스(예약→확정→상담기록→전환집계)를 end-to-end로 증명하는 것이다.

**기술 스택 (ADR 0001, ADR 0005)** — 아래 표는 레이어별 채택 기술과 그 근거를 정리한다.
...
> **근거**: 초기 설계는 Vite + React SPA를 상정했으나, JWT를 브라우저 JS가 직접 보유하는 구조는 XSS 노출 위험이 있다.
>
> - Next.js App Router를 채택함으로써 JWT를 httpOnly 쿠키에 격리한다.
> - Next.js Middleware에서 역할 기반 라우트 보호를 선언적으로 처리한다.
> - 추가 복잡도(SSR, API Route Handler 프록시) 대비 보안 격리와 역할 보호 단순화 이득이 더 크다고 판단했다.
> - 초기 Vite 소스는 이후 제거되어 현재 구현은 단일 Next.js 앱이다(이중 코드베이스 없음).

### 1.2 아키텍처 다이어그램

> **보는 법**: 위에서 아래로 프론트엔드(Next.js) → 백엔드(NestJS 모듈) → DB(PostgreSQL) 3개 층이다. 점선 경계(`NotificationChannel`, `EmailCh`/`SmsCh`)는 어댑터 경계와 Phase 2 stub을 표시한다.
...
**한 줄 요약**: 전체 엔티티와 관계를 한 장에 담은 ERD다. subject 파생(`subjectType=CUSTOMER` + `subjectId`)과 대칭형 `FamilyLink` 설계는 [§4](#4-subject-파생-및-가족-연결-설계)에서 상세 설명한다.

> **보는 법**: 핵심 흐름은 `Customer → Booking → ConsultationRecord`이며, `Booking`은 `TestResult`(예약 근거)와 `AvailabilitySlot`(시간)을 연결한다. `FamilyLink`는 두 `Customer`를 대칭으로 잇고, Analytics용 조인 테이블(`ConsultationRecordMetric`·`ConsultationRecordProduct`)이 지표·상품별 전환 집계의 근거다.
...
erDiagram
    ...
    CallLog {
        ...
        cuid bookingId FK
        cuid counselorId FK
        enum outcome "CONNECTED|NO_ANSWER|INVALID"
    ...
    Booking ||--o{ CallLog : "1-N (cascade)"
    Counselor ||--o{ CallLog : "1-N (cascade)"
```
...
**한 줄 요약**: 위 ERD에서 설계 의도를 읽어야 하는 컬럼·엔티티를 하나씩 풀어 설명한다.

**`AvailabilitySlot.isOpen`의 역할 한정**: `isOpen`은 상담사가 슬롯을 운영상 열고/닫는 플래그다. "예약됨/비어있음"을 표현하지 않는다. 가용성은 파생 쿼리로 계산한다 ([§6](#6-가용성-파생값-설계)).

**`Booking.status` 5-상태 열거형**: 전이는 `PENDING → CONFIRMED → COMPLETED | NO_SHOW` 또는 `PENDING/CONFIRMED → CANCELLED`다.

- `NO_SHOW`는 ops-hardening(ADR 0006)에서 추가된 단말 상태다. 슬롯 종료 후 상담사가 기록을 남기지 않으면 스케줄러(`sweepNoShows`)가 상태 가드 `updateMany`로 전이한다.
- `NO_SHOW` 예약은 `/counselor/schedule`에 포함되어 상담사 콘솔에서 검토·필터링된다(`CANCELLED`만 제외; ADR 0013).
- 부분 unique 인덱스는 `status IN ('PENDING','CONFIRMED')`에서만 적용되어 PENDING 상태도 슬롯을 선점한다.

**`Booking.testResultId`**: 예약의 근거가 된 TestResult.

- 서버가 이 필드로 subject(`subjectType=CUSTOMER`, `subjectId`=검사결과 소유 고객)를 파생한다.
- 따라서 클라이언트가 잘못된 subject를 직접 지정하는 것을 방지한다.

**`Booking.slotStartAt` / `Booking.slotEndAt`**: 예약된 슬롯의 `[startAt, endAt)` 윈도우를 비정규화 복제한 값이다.

- 목적: 고객 시간대 중복 방지 GiST `EXCLUDE` 제약(`booking_customer_no_overlap`)이 단일 `Booking` 행에 대해 시간 범위를 만들 수 있도록 한다(시간 정보는 본래 `AvailabilitySlot`에 있음).
- `create()` 시점 write-once이며, 예약은 슬롯을 재조정하지 않으므로 표류하지 않는다.
- 향후 reschedule 기능 도입 시 이 컬럼을 슬롯과 함께 갱신해야 한다(ADR 0015).

**`ConsultationRecord` 구조화 슬롯**: 자유 텍스트 단일 `notes`를 3슬롯으로 분리했다.

- 3슬롯: `summary`(주요 상담 내용 — 필수), `recommendation`(권고 사항 — 필수), `followUp`(후속 조치 — 선택).
- 마이그레이션(`structured_consultation_record`): enum 생성 → 컬럼 추가 → 기존 `notes`에서 summary 백필 → NOT NULL 설정 → `notes` 드롭.
- 고정 슬롯은 상담사 간 기록 형태를 강제해 일관성을 보장한다.
...
**`ConsultationActionType` 다중 선택 체크리스트**: 상담 기록의 `actions[]`에 다중 선택으로 저장되는 상담행위 enum이다.

- 값: `METRIC_EXPLAINED`(지표 설명), `DIET_GUIDANCE`(식이 권고), `SUPPLEMENT_GUIDANCE`(영양제 권고), `RETEST_GUIDANCE`(재검사 안내), `LIFESTYLE_GUIDANCE`(생활습관 안내).
- `structured_consultation_record` 마이그레이션에서 도입(동시에 `notes` 드롭).
- `outcome` enum과 마찬가지로 enum 제약으로 상담사 간 기록 일관성을 강제한다.
- `actions × outcome`이 쿼리 가능해 어떤 상담행위가 전환에 기여했는지 상담행위별 전환 분석을 할 수 있다.

**`FamilyLink`**: 두 `Customer` 계정 사이의 **대칭형**(Customer↔Customer) 초대 코드 기반 연결이다.

- `inviterCustomerId`가 초대 코드를 생성하고, 수락 시 `inviteeCustomerId`가 설정된다.
- `customerLowId`/`customerHighId` 정규화 쌍으로 ACCEPTED 쌍 중복을 partial unique 인덱스로 차단한다.
- TestResult 조회(`GET /test-results`)와 예약 소유권 검증은 ACCEPTED 상태의 FamilyLink로 연결된 파트너의 검사결과를 포함한다.
- 가족 구성원도 자체 `Customer` 계정을 보유하며, 별도의 `FamilyMember` 엔티티는 없다.

**`TestResult.serviceType` + `metrics` 형태(BioCom — ADR 0007)**: `serviceType`은 자유 문자열을 유지하되 시드 상수로 표류를 차단하고, `metrics`는 하위호환 superset으로 확장됐다.

- `serviceType`은 자유 문자열을 유지(ADR 0007: seed-only/read-only)하되, seed와 미래 필터가 공유하는 단일 `SERVICE_TYPES` 상수(7종: `METABOLIC_6`·`FOOD_INTOLERANCE`·`STRESS_AGING`·`NUTRIENT_HEAVY_METAL`·`GUT_MICROBIOME`·`HORMONE`·`PET_NUTRITION`)에서 코드를 가져와 표류를 차단한다.
- `metrics` JSONB 배열 요소는 `{metricKey, label?, value, unit?, referenceRange?, status?}`로 **하위호환 superset** 확장됐다 — 기존 소비자(`normalizeMetrics`, `toMetricList`)는 `{metricKey,value,unit}`만 읽고 새 키를 무시하므로 무영향.
- `status`/`referenceRange`는 결과 해석 UX를 위해 시드에 사전 계산되며(`status ∈ {정상,주의,위험}`), enum 대신 문자열을 쓴 것은 마이그레이션·테스트 픽스처 churn을 피하기 위함이다.

**`Challenge` + `ChallengeEnrollment`(BioCom step-3 — ADR 0007)**: `Challenge`는 시드 카탈로그, `ChallengeEnrollment`는 상담사가 상담 기록 생성 시 고객을 등록하는 join 엔티티다.

- `Challenge`는 시드 관리 카탈로그(`GET /challenges`)다.
- 등록은 `createRecord` 트랜잭션 안에서 원자적으로 생성된다(`updateRecord`는 등록을 건드리지 않음).
- 존재하지 않는 `challengeId`는 트랜잭션 진입 **전** `findUnique` 가드가 깨끗한 404로 막는다.
- 코드는 outcome에 게이팅하지 않으며(어떤 outcome도 등록 가능), 구매와의 연관은 UI 컨벤션·Analytics 해석에서만 표현된다.
- `counselorId`는 비정규화 저장하되 Analytics 전환율은 항상 record JOIN(`ConsultationRecord.counselorId`)으로 범위를 산정한다.

**`Booking.concern` + `Booking.briefOpenedAt`**: 고객 사전질문과 상담사 브리핑 열람 시각을 보관한다.

- `concern`은 고객이 예약 생성 시 선택적으로 전달하는 사전질문(`@MaxLength(1000)`)이다. 브리핑 조립 시 상담사에게만 노출되며 고객 API로 반환하지 않는다(write-only).
- `briefOpenedAt`은 상담사가 브리핑을 최초 열람한 시각이다. 조건부 `updateMany({ where: { id: bookingId, briefOpenedAt: null } })`로 1회만 기록된다(DB 레이어 멱등 — 동시 열람 경쟁에서도 최초 1회만 설정).

**`ConsultationBriefGuidance` + `BriefGuidanceStatus`**: 다가오는 상담을 **어떻게 진행할지**에 대한 상담 전 가이던스를 예약 단위로 보관한다(대상자 검사 지표 + 과거 상담 기록 + `concern`에서 파생, 사후 요약 아님).

- **보장**: 상담사가 브리핑을 열면(`getBookingBrief`) `GuidanceService.ensureFallbackForBooking(bookingId)`가 결정론 템플릿 가이던스를 `status=FALLBACK`으로 보장한다(없으면 생성, 있으면 유지).
- **멱등 타깃**: `bookingId @unique`가 스윕의 멱등 업서트 타깃이고 `Booking` 1:1 관계는 `onDelete: Cascade`다.
- **업그레이드 스윕**: OpsScheduler `@Interval` `sweepPendingUpgrades()`가 `status=FALLBACK` 행만 조회 → 로컬 Ollama 도달 가능 시 `gemma4:e4b`로 생성 → `UPGRADED`로만 업서트(절대 downgrade 없음).
- **fail-soft**: Ollama 미설치·실패 시 FALLBACK 유지.
- **분리**: `createRecord`는 가이던스를 건드리지 않으며, 가이던스 생명주기는 상담 기록과 완전히 분리된다(ADR 0014).

**`ConsultationRecordMetric`**: 상담 기록과 검사 지표를 연결하는 조인 테이블이며, "지표별 전환율" 집계의 근거다.

- `metricKey`는 `TestResult.metrics` JSONB 배열 내 지표 식별자다.
- Analytics는 이 테이블을 조인해 "지표별 전환율"을 집계한다 — 단순 상담 CRM과의 차별점(R5).
- 조인 테이블은 `label` 없이 영문 `metricKey`만 저장하므로(저장 비정규화 회피), 대시보드 표시 계층(`지표별 구매 전환` 표·`연계 지표` 배지)은 프론트엔드 라벨 맵 `formatMetricKey()`(`frontend/src/entities/test-result/lib/metrics.ts`, `@/entities/test-result`로 노출)로 `glucose → 공복혈당` 한글 라벨로 변환한다 — 미등록 키는 키 원문으로 폴백.
- 지표 카탈로그가 seed 고정이라 정적 맵으로 충분하다(`formatServiceType`와 동일 패턴).

---

## 3. 예약 생명주기 상태 다이어그램

**한 줄 요약**: 예약은 PENDING으로 시작해 상담사 확정·완료·노쇼·취소로 전이한다. PENDING도 슬롯을 선점하는 것이 핵심이다.

> **근거**: 초기 설계는 예약 생성 즉시 `CONFIRMED`로 설정하는 단순 모델이었다.
>
> - 실제 상담 운영에서는 상담사가 일정을 확인하고 수락하는 절차가 필요하다는 점에서 PENDING-first 모델로 변경했다.
> - PENDING 상태도 슬롯을 선점함으로써 TOCTOU 없이 동시성을 보장한다.

> **보는 법**: 각 화살표는 상태 전이를 트리거하는 엔드포인트(또는 스케줄러)다. 오른쪽 note 3개는 부분 unique 인덱스 범위, 취소 시 재노출, 단말 상태 멱등성을 각각 설명한다.
...
**한 줄 요약**: 상담 대상이 예약자 본인이 아닐 수 있는 문제를, 다형 참조 대신 CUSTOMER-only subject + 대칭형 FamilyLink로 푼다.

### 4.1 문제 정의

- 상담 대상(subject)이 예약 계정 소유자와 다를 수 있다(가족 검사결과 대리 상담, R3).
- 핵심은 "예약자가 어떤 검사결과를 상담 대상으로 지정할 수 있는가"를 안전하게 표현·검증하는 것이다.

설계 변천:

- 초기 설계(ADR 0003)는 `subjectType ∈ {CUSTOMER, FAMILY_MEMBER}` + `subjectId` enum+id 페어로 `Customer` 또는 `FamilyMember`를 다형 참조했다.
- 이후 **대칭형 `FamilyLink` 재설계**(`symmetric_family_link` 마이그레이션)로 `FamilyMember` 1급 엔티티를 제거하고, 가족 구성원도 자체 `Customer` 계정을 보유하도록 단순화했다.
- 그 결과 **subject는 항상 `CUSTOMER`**이며, `subjectId`는 검사결과를 소유한 고객(본인 또는 `ACCEPTED` `FamilyLink` 파트너)을 가리킨다.
...
**한 줄 요약**: DB FK만으로는 자격을 보장하지 못하므로, 서비스 레이어 소유권 가드가 "본인 또는 ACCEPTED 파트너"를 검증한다.

- subject가 단일 테이블을 가리키더라도 DB FK만으로는 "예약자가 이 고객의 검사결과를 쓸 자격이 있는가"를 보장하지 못한다.
- 따라서 서비스 레이어 소유권 가드가 이를 검증한다.
...
**한 줄 요약**: 동일 슬롯 동시 예약을 DB unique constraint + insert-first로 막아 TOCTOU를 구조적으로 제거한다.

### 5.1 문제 정의

- 동일 슬롯에 N명이 동시에 `POST /bookings`를 보낼 때 단 1건만 ACTIVE(PENDING 또는 CONFIRMED)되어야 한다.
- 잘못 설계하면 중복 예약이 DB에 저장되며, 이는 서비스 신뢰성 붕괴다.
...
> **보는 법**: 아래 표는 동시성 보장 3전략을 원자성 주체·복잡도·확장성으로 비교한다. 굵게 표시된 첫 행이 채택안이다.
...
**한 줄 요약**: 슬롯 unique는 같은 슬롯만 막으므로, 한 고객의 겹치는 시간대 복수 예약은 GiST `EXCLUDE` 제약으로 따로 막는다.

문제:

- `booking_slot_active_unique`는 **동일 슬롯**의 중복만 막는다.
- 한 고객이 **서로 다른 슬롯**(예: 서로 다른 상담사)의 같은/겹치는 시간대에 복수 ACTIVE 예약을 보유하는 것은 막지 못한다 — 사람은 동시에 두 상담을 받을 수 없으므로 이는 무결성 위반이다.
- 슬롯 길이가 가변(30/60분)이라 시작 시각 동일이 아닌 **시간 범위 겹침**으로 강제해야 한다.
...
**한 줄 요약**: 가용성은 컬럼이 아니라 파생 쿼리값이다. `isOpen`은 운영 플래그로만 쓰고, 동일 시간대는 통합 캘린더로 묶어 `availableCount`로 표현한다.

### 6.1 단일 진실원 원칙

- `isOpen`은 상담사 운영 플래그로만 한정한다. "예약됨/비어있음"은 `isOpen`이 표현하지 않는다.
- 이중 진실원 제거로 `isOpen=true`이면서 동시에 ACTIVE Booking이 존재하는 불일치 버그 원천을 차단한다.
...
> **보는 법**: 미래·`isOpen` 슬롯에서 ACTIVE Booking과 업무시간 필터를 거쳐 가용 풀을 만들고, 같은 시간대는 상담사 수만큼 `availableCount`로 합산해 단일 엔트리로 응답한다.
...
**한 줄 요약**: 두 고객은 초대 코드 수락이라는 명시적 동의로만 연결되며(대칭형), ACCEPTED 링크만 서로의 검사결과 접근을 연다.

### 7.1 문제 정의

- 가족 구성원도 각자 독립 `Customer` 계정을 가진다. 두 고객이 연결되지 않으면 서로의 TestResult를 대리 예약에 사용할 수 없다.
- 반대로 임의의 두 고객이 자동으로 서로의 검사결과에 접근 가능해지면 프라이버시 경계가 없어진다.
- 따라서 양측의 명시적 동의(초대 코드 수락)로 성립하는 **대칭형** 연결이 필요하다.

### 7.2 FamilyLink 상태 전이

> **보는 법**: PENDING(코드 생성) → ACCEPTED(상대 수락) → REVOKED(해제)가 주 경로이며, PENDING은 `expiresAt` 경과로 만료된다.
...
**한 줄 요약**: `GET /test-results`는 본인과 ACCEPTED 파트너의 검사결과만 반환한다(PENDING/REVOKED 제외).
...
**한 줄 요약**: 권한은 역할 RBAC 층과 자원 소유권 층으로 분리하며, 두 층은 방어 심층화 관계로 서로를 대체하지 않는다.

- 권한은 **역할(role) 층**과 **자원 소유권(ownership) 층**으로 분리한다.
- 이 두 층은 별개의 관심사이며, 한 층이 다른 층을 대체하지 않는다.

### 8.1 두 층의 책임과 한계

> **보는 법**: 아래 표는 각 층이 "막는 것"과 "막지 못하는 것"을 대비시킨다. 역할 층이 못 막는 것을 소유권 층이 막고, 그 반대도 성립한다.
...
**한 줄 요약**: Next.js App Router 위에 Feature-Sliced Design(FSD)을 얹고, JWT는 httpOnly 쿠키 + Middleware로 보호한다.

### 9.1 Next.js App Router 채택 이유

> **보는 법**: 아래 표는 기각된 Vite SPA(초기 설계)와 채택안 Next.js App Router를 4개 축으로 대비한다.
...
**한 줄 요약**: 단방향 import 레이어(`app → views → widgets → features → entities → shared`)로 의존 방향을 강제하고, 슬라이스 간 참조는 Public API 배럴만 경유한다.

- 프론트엔드는 **Feature-Sliced Design**으로 구성된다.
- 모든 레이어는 `src/` 아래 있고, import 방향은 한 방향이다: `app(라우팅) → views → widgets → features → entities → shared`.
- 슬라이스 간 참조는 각 슬라이스 `index.ts`(Public API)만 경유한다(deep-import 금지).
...
- **세그먼트 — `types`/`constants` 분리 (ADR 0012)**: 타입과 모듈 수준 상수를 전용 세그먼트로 분리한다.
  - 슬라이스의 모든 타입·인터페이스 선언은 `types/`, 모듈 수준 상수(색/라벨 맵 등)는 `constants/`에 둔다.
  - entity의 `model/`은 타입 전용이었으므로 `types/`로 일원화하며 제거한다.
  - 로직이 있는 feature `model/`(`complete-booking`·`create-consultation-record`)은 유지하되 타입만 추출한다.
  - `ui/*.tsx`의 인라인 Props·상수도 각각 `types/`·`constants/`로 분리한다(`lib` 유틸 내부 전용 타입/상수는 co-locate 유지).
  - 배럴 공개 API는 불변.

### 9.2.2 스타일링·상태·API 훅 레이어 (ADR 0011)

**한 줄 요약**: 스타일링은 Tailwind on Radix, 상태는 서버=TanStack Query / 클라이언트=Jotai, API는 슬라이스별 배럴 훅 — 세 가지 횡단 컨벤션을 둔다.

- **스타일링 — Tailwind on Radix**: Radix Themes는 유지하고 Tailwind를 그 위에 레이어링한다.
  - Tailwind가 레이아웃·간격·타이포·일회성 스타일을 담당한다.
  - 색/반경은 `tailwind.config.ts`에서 **Radix 런타임 CSS 변수에 매핑**(`text-teal-11` → `var(--teal-11)`)되어 값 중복 0·테마 자동 추종.
  - Radix가 자체 reset을 가지므로 Tailwind **preflight는 OFF**(맨 `border`는 항상 색과 함께).
  - 반복 토큰 조합은 `shared/ui` 프리미티브(`Eyebrow`·`StatNumber`·`Meter`)로 1회 정의한다.
  - 동적 톤 색은 정적 클래스 맵(`shared/ui/tone.ts`)으로 변환(템플릿 문자열 클래스는 purge되므로 금지).
  - `globals.css`는 keyframes·로그인 아트 등 진성 CSS만 유지하고, 컴포넌트 인라인 `var()` 스타일은 제거.
- **상태 — 서버=TanStack Query, 클라이언트=Jotai**: `providers.tsx`가 두 Provider를 모두 감싼다(Jotai Provider는 요청별 store 격리로 SSR 안전).
  - 서버 상태는 TanStack Query, 공유 클라이언트 상태는 Jotai atom.
  - 로컬 UI 상태는 `useState` 유지(over-적용 금지).
- **API 훅 레이어**: 각 entity `api/queries.ts`에 `<slice>Keys` queryKey 팩토리 + `useX()`/`useXMutation()` 래퍼를 두고, 저수준 fetcher를 감싼다.
  - 뷰/피처는 인라인 `useQuery`/`useMutation` 대신 슬라이스 배럴 훅을 소비한다(뮤테이션 훅이 무효화 책임, 호출부는 per-call `onSuccess`로 UI 부수효과 보존).
  - `queryKey`·옵션은 인라인 원본과 동일하게 보존(캐시 동작 무변경).

### 9.3 JWT 흐름 (프론트엔드)

**한 줄 요약**: 로그인 시 JWT는 httpOnly 쿠키로만 내려가고, Middleware(1차)와 NestJS(2차)가 서명·역할·소유권을 단계별로 검증한다.

> **보는 법**: 위쪽은 로그인 → 쿠키 설정, 아래쪽은 보호 페이지 접근 시 Middleware 검증과 `/api/proxy` 헤더 변환 흐름이다.
...
**근거 (방어 심층화, ADR 0010)**: Middleware와 NestJS가 각각 1차·2차 접근제어 층을 이루는 방어 심층화 구조다.

- **1차 (Middleware)**: `src/middleware.ts`는 쿠키 JWT의 **서명과 만료를 `jose`(HS256)로 검증**한 뒤 역할을 확인한다 — 백엔드와 동일한 `JWT_SECRET` 사용.
- 따라서 미인증·위조·만료 쿠키는 보호 페이지의 정적 shell조차 받지 못하고 `307 /login`으로, 역할이 맞지 않으면 본인 홈으로 리다이렉트된다(서버측 접근제어 경계).
- 이는 단순 디코드 기반 UX 필터가 아니라 실제 1차 접근제어 층이다.
- **2차 (NestJS)**: 모든 API 요청에서 서명 검증 + **자원 소유권**까지 강제한다(데이터 보안 경계).
- 두 층은 상호 대체가 아니라 방어 심층화 관계다.
- 검증 가능한 순수 로직(서명 검증 `shared/auth/verify.ts`, 라우트→역할 매핑 `shared/auth/access.ts`)은 단위 테스트로 고정한다.

---

## 10. Golden-path 시퀀스 다이어그램

**한 줄 요약**: 메인 골든 패스(예약→확정→상담기록→집계)와 동시 충돌·만석 대안 경로를 시퀀스 다이어그램으로 증명한다.

### 10.1 메인 골든 패스 — 예약→확정→상담기록→집계

> **보는 법**: 번호 1~6 블록이 시간순 단계다. 1 로그인 → 2 캘린더 → 3 PENDING 예약 → 4 상담사 확정/브리핑 → 5 상담 기록(+가이던스 스윕) → 6 관리자 전환 집계.
...
> **보는 법**: 20개 동시 INSERT를 부분 unique 인덱스가 중재해 1건만 201, 나머지는 SQLSTATE 23505 → 409로 매핑된다. 최종 DB ACTIVE Booking count는 1이다.
...
> **보는 법**: 고객2는 만석 시 가용 캘린더에서 다른 일자·상담사를 탐색하고, 고객1의 취소는 슬롯을 부분 unique 인덱스 조건에서 빼 promotion 없이 즉시 재노출시킨다.
...
**한 줄 요약**: 핵심 엔드포인트의 역할·요청·응답·에러를 표로 명세하며, OpenAPI 3.0 스펙은 `@nestjs/swagger`로 자동 생성된다.
...
> **보는 법**: 역할 열은 RBAC 게이트(§8.2), 주요 에러 열은 §11.2 상태 코드와 대응한다. 핵심 골든 패스 엔드포인트는 `POST /bookings` → `PATCH /bookings/:id/confirm` → `POST /consultation-records` → `GET /admin/analytics`다.
...
| `GET`    | `/counselor/bookings/:bookingId/brief` | COUNSELOR           | —                                                                                                                                                                                                                            | `BookingBrief` (`indicators[]`(metricKey asc, 이상 플래그), `pastRecords[]`(createdAt desc), `familyContext?`, `concern?`, `guidance`(상담 전 가이던스 — FALLBACK/UPGRADED)) — 최초 호출 시 `briefOpenedAt` 1회 기록 + FALLBACK 가이던스 보장, `phone: string`(고객 연락처 — COUNSELOR 소유권 검증 범위)                                                                                  | 403 (타 상담사 예약), 404                                                                                        |
| `POST`   | `/counselor/bookings/:bookingId/calls` | COUNSELOR           | `{ outcome: CONNECTED\|NO_ANSWER\|INVALID, note?: string }`                                                                                                                                                                  | `CallLog` (201; bookingId·counselorId·outcome·note·createdAt)                                                                                                                                                                                                                                                                  | 403 (타 상담사 예약), 404                                                                                        |
...
| `GET`    | `/admin/analytics`                     | ADMIN, COUNSELOR    | Query: `?scope=own\|all&counselorId=uuid`                                                                                                                                                                                    | `{ conversionRate, outcomeDistribution, productInterest[], metricConversion[], challengeEnrollments, challengeConversionRate, contactAttempts, callOutcomeDistribution, noShowWithoutContactRate }` (`challengeConversionRate: number\|null`; `noShowWithoutContactRate: number\|null`)                                                                                                                                                      | 401, 403                                                                                                         |
...
> **보는 법**: 본문 전체의 설계 결정을 채택안·핵심 근거·관련 ADR로 한 표에 압축한 색인이다.
...
**한 줄 요약**: 고객이 자기/가족 검사결과에 대해 질문하면, 스코프 게이트 → 소유권 라이브 재검증 → Ollama(또는 결정론 폴백)로 해석을 돌려주는 모듈이다. 골든 패스는 Ollama 없이도 항상 통과한다.
