...
erDiagram
    ...
    Notification {
        ...
        cuid waitlistId FK "nullable"
        enum type "CONFIRMATION|REMINDER|SLOT_OPENED"
    ...
    Waitlist {
        ...
        cuid counselorId FK
        enum subjectType "CUSTOMER (항상)"
        cuid subjectId "검사결과 소유 고객"
        enum status "WAITING|NOTIFIED|CONVERTED|EXPIRED|CANCELLED"
        cuid offeredSlotId FK "nullable — advisory 오퍼 슬롯"
        datetime offerExpiresAt "nullable — advisory TTL"
        datetime createdAt
    }
...
stateDiagram-v2
    ...
    note right of CANCELLED
        취소 시 동일 상담사
        WAITING Waitlist 1건
        NOTIFIED 전이 + SLOT_OPENED 알림
        (ADR 0006: advisory 오퍼)
    end note
...
**비관적/낙관적 락 비채택 이유**: 단건 슬롯 예약에서 추가 복잡도 대비 이득이 없다. 슬롯을 시간 단위로 분할하거나 waitlist 자동매칭(R8, Phase 2)으로 확장될 때 재검토 항목으로 ADR에 기록한다.
...
if (subjectType !== SubjectType.CUSTOMER) {
  throw new ForbiddenException('Subject does not belong to the current customer');
}
...
const booking = await tx.booking.findUnique({ where: { id: dto.bookingId }, include: { slot: true } });
if (booking.slot.counselorId !== requestCounselorId) {
  throw new ForbiddenException('본인 담당 예약에만 상담 기록을 작성할 수 있습니다.');
}
```
...
├── entities/   # 12개 도메인 슬라이스 (types·constants·api·ui) — session·booking·availability·schedule·
│               #   waitlist·test-result·consultation-record·analytics·family-link·notification·product·challenge
...
### 10.3 Waitlist FIFO — 취소 → NOTIFIED + SLOT_OPENED

```mermaid
sequenceDiagram
    actor Customer2 as 대기 고객 (waitlist)
    actor Customer1 as 예약 고객 (취소)
    participant Booking as BookingService
    participant Waitlist as WaitlistService
    participant Notif as NotificationModule
    participant DB as PostgreSQL

    Note over Customer2, DB: 사전: 슬롯 만석, 고객2 대기열 등록
    Customer2 ->> Booking: POST /waitlist { counselorId, subjectType, subjectId }
    Booking ->> DB: INSERT waitlist(status=WAITING)

    Note over Customer1, DB: 예약 취소 (PENDING 또는 CONFIRMED)
    Customer1 ->> Booking: DELETE /bookings/:id
    Booking ->> DB: BEGIN TX\nUPDATE booking SET status=CANCELLED
    Booking ->> DB: SELECT waitlist WHERE counselorId=? AND status=WAITING\nORDER BY createdAt ASC LIMIT 1
    DB -->> Booking: Waitlist { id: W1, customerId: C2 }
    Booking ->> DB: UPDATE waitlist SET status=NOTIFIED WHERE id=W1
    Booking ->> Notif: SLOT_OPENED 알림 생성 (waitlistId=W1)
    Notif ->> DB: INSERT notification(type=SLOT_OPENED, PENDING→SENT)
    DB -->> Booking: COMMIT
    Booking -->> Customer1: 200 OK
    Note over Customer2: GET /notifications → SLOT_OPENED 알림 수신
```
...
| 코드 | 의미 | 대표 케이스 |
|------|------|------------|
| 201 | 생성 성공 | POST /bookings, POST /consultation-records, POST /waitlist |
...
| 운영 강건화 | 현존 모듈 확장 (no-show 스윕·waitlist 전환·운영 퍼널·슬롯 CRUD) | 최소 표면적, 검증된 패턴 재사용, additive enum/컬럼 | [0006](./05-adr/0006-ops-hardening.md) |
