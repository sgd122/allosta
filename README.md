...
상담사 생산성을 높이는 **사전 브리핑 + 사후 AI 요약** 기능이 포함됩니다. AI 요약은 로컬 Ollama 없이도
항상 결정론 FALLBACK 텍스트로 제공되며, Ollama를 설치하면 ~5s 내 UPGRADED 요약으로 교체됩니다.
...
| 4 | **시스템 설계 문서** | [`docs/04-system-design.md`](docs/04-system-design.md) (ERD·아키텍처·시퀀스·API·동시성) + [`docs/05-adr/`](docs/05-adr/) ADR 14종 |
...
NestJS (:3000) ──Prisma──▶ PostgreSQL (:5432, docker)
  ...
  │     ├── availability-calendar (시간대 집계, availableCount)
  ...
  │     └── SummaryModule (createRecord 후 FALLBACK 영속 → OpsScheduler 스윕으로 UPGRADED)
  ├── Analytics (scope 토글: own/all + 운영 퍼널/지표 + 챌린지 등록 수/전환율 + briefOpenRate + aiSummary 지표)
  ├── Waitlist (FIFO 공석 알림 + TTL advisory 오퍼 → CONVERTED/EXPIRED 전환 추적)
  ├── OpsScheduler (@Interval 타이밍 래퍼 — sweepNoShows / sweepWaitlistOffers / sweepPendingUpgrades)
...
- **동시성**: `booking_slot_active_unique` 부분 unique 인덱스(`WHERE status IN ('PENDING','CONFIRMED')`) + **insert-first → catch P2002 → 409** 패턴. PENDING 상태도 슬롯 선점.
...
- **운영 강건화 (ops-hardening, ADR 0006)**:
  ...
  - **Waitlist 전환**: 취소 발생 시 첫 대기자에게 TTL advisory 오퍼(`offeredSlotId`/`offerExpiresAt`) 발행 → `sweepWaitlistOffers`가 만료분을 `EXPIRED`로, 실제 예약 성사 시 `CONVERTED`로 전환 추적.
  - **운영 퍼널 Analytics**: `GET /admin/analytics`에 `funnel{booked,confirmed,completed,noShow,cancelled}` + `noShowRate` + `slotUtilization` + `waitlistConversionRate` 추가.
...
- **상담 준비·생산성 자동화 (ADR 0014)**:
  ...
  - **AI 요약 생명주기**: `createRecord` 커밋 직후 결정론 템플릿 요약을 `ConsultationAiSummary(status=FALLBACK)`으로 동기 영속(Ollama 의존 없음). OpsScheduler `@Interval(5000)` `sweepPendingUpgrades()`가 `status=FALLBACK` 행만 Ollama로 업그레이드 → `UPGRADED`로만 업서트(절대 downgrade 없음, 멱등).
  - **Ollama 옵트인**: Ollama 없이도 golden path는 항상 통과(FALLBACK이 기본). Ollama 설치 후 `ollama pull gemma3n:e4b`를 실행하면 ~5s(1 스윕 사이클) 내에 UPGRADED 요약이 노출된다. 환경 변수 미설정 시 기본값(`OLLAMA_BASE_URL=http://localhost:11434`, `SUMMARY_MODEL=gemma3n:e4b`) 사용, startup assertion 없음.
  - **수동 스윕 트리거**: `POST /admin/summary/sweep` (`@Roles ADMIN`) → `{ upgraded }`. `@Interval` 5s를 기다리지 않고 데모에서 즉시 UPGRADED 확인 가능.
  - **Analytics**: `briefOpenRate`(분모=CONFIRMED+COMPLETED+NO_SHOW) + `aiSummaryCount` + `aiSummaryUpgradedRatio`. 기존 scope 토글 준수.
...
1. **고객**으로 로그인 → `/book` 슬롯 예약 화면.
   ...
   - (슬롯이 모두 차 있으면 **Waitlist** 등록 가능 — 취소 발생 시 FIFO 공석 알림.)
2. **상담사**로 로그인 → `/schedule` 본인 일정 확인 → 예약 **확정** (`PENDING → CONFIRMED`).
   ...
   - **브리핑 패널**: 예약 상세에서 브리핑 패널을 열면 검사 지표(이상 플래그 포함)·과거 상담기록·가족 맥락·고객 `concern`이 자동 조립되어 표시된다. 최초 열람 시각(`briefOpenedAt`)이 기록되며 브리핑 열람률 지표의 분자가 된다.
   ...
   - 기록 저장 즉시 **AI 요약(FALLBACK)**이 생성된다. Ollama가 실행 중이면 ~5s 내 UPGRADED 배지로 전환된다.
...
3. **고객**(또는 가족 계정)으로 다시 로그인 → 슬롯이 찼던 시간대에 취소가 발생하면 `/bookings`에 **waitlist 오퍼 카드**가 노출 → TTL 내 예약 시 `CONVERTED`, 미응답 시 `EXPIRED`. (오퍼 카드의 **예약 취소** 버튼으로 직접 취소 가능.)
4. **관리자**로 로그인 → `/dashboard` → 전환율·outcome 분포·상품별 관심·**지표별 전환**(영문 `metricKey`를 `공복혈당` 등 한글 라벨로 표시)에 더해 **운영 퍼널 카드**(booked/confirmed/completed/noShow/cancelled), **운영 지표 카드**(noShowRate·slotUtilization·waitlistConversionRate), **챌린지 카드**(등록 수 + 등록 전환율 — null=구매 0건이면 "—", 0이면 "0%"), **브리핑 생산성 카드**(briefOpenRate + aiSummaryCount + aiSummaryUpgradedRatio)가 실시간 집계로 표시.
   - **AI 요약 수동 스윕**: `POST /admin/summary/sweep`(또는 대시보드 "스윕 실행" 버튼)으로 FALLBACK→UPGRADED 즉시 트리거 가능(`{ upgraded }` 반환).
...
백엔드 통합 테스트 **18 suites / 122 tests** (AC-P1~P7 상담 준비 자동화 포함). 어떤 assertion도 전체 스위트 개수를 하드코딩하지 않습니다(green/red로 회귀 판정). 프론트엔드 vitest: **14 files / 75 tests**(세션 JWT 서명 검증 `shared/auth/verify.test.ts`·라우트 접근 정책 `shared/auth/access.test.ts`·상담사 콘솔 일정 필터·그룹핑 순수 함수 `shared/lib/date`·`views/schedule/lib/filter`·`views/availability/lib/grouping` 포함).
...
| `golden-path.e2e-spec.ts` | 예약→확정→기록(+챌린지 등록)→집계 흐름 + 지표별 전환(AC9) + 상태 배지 가시성 + 챌린지 등록/전환 셀 + waitlist 공석 알림(AC10) | AC1, AC4, AC6, AC9, AC10, AC-C, AC-M |
...
| `waitlist-conversion.spec.ts` | TTL advisory 오퍼 발행, `sweepWaitlistOffers` EXPIRED 만료, 예약 성사 시 CONVERTED 전환 | AC-W2, AC-W3, AC-W4, AC-W5 |
| `analytics-ops.spec.ts` | 운영 퍼널(booked/confirmed/completed/noShow/cancelled) + noShowRate·slotUtilization·waitlistConversionRate 집계 | AC-A1 |
...
│   │   │                           # symmetric_family_link · ops_enum_add · waitlist_offer_fields ·
...
│   │   ├── waitlist/               # FIFO 공석 알림, TTL 오퍼, CONVERTED/EXPIRED 스윕
│   │   ├── consultation/           # 구조화 기록, metricRefs, 챌린지 카탈로그/등록 (createRecord 원자)
│   │   │   ├── summary/            # SummaryModule — TemplateSummarizer, OllamaSummarizer, SummaryService
│   │   │   └── brief (getBookingBrief — 결정론 조립, briefOpenedAt 멱등 기록)
│   │   ├── analytics/              # 전환 집계, scope 토글, 운영 퍼널/지표, 챌린지 등록 수/전환율, briefOpenRate/aiSummary 지표
│   │   ├── test-result/            # seed + read-only, /my 엔드포인트 (BioCom 7종 + 참조범위/상태)
│   │   ├── common/constants/       # SERVICE_TYPES 공유 상수 (BioCom 7종 코드)
│   │   ├── ops-scheduler/          # @Interval 타이밍 래퍼 (sweepNoShows / sweepWaitlistOffers)
...
└── frontend/                       # Next.js 14 App Router · FSD(ADR 0009, types/constants 세그먼트 0012) · Tailwind on Radix + Jotai(ADR 0011)
    ...
    └── src/
        ...
        │   ├── (customer)/book (결과서 단위 선택) · bookings (waitlist 오퍼 카드 · 예약 취소 · 친화 라벨) · results (내 검사/연동 계정 서브탭 · 결과서 그룹핑 · 참조범위 컬럼 + 상태 배지)
        ...
        ├── entities/               # 13개 도메인 슬라이스 (types · constants · api: fetcher+queries.ts 훅 · ui) — 타입/상수 세그먼트 분리(ADR 0012). consultation-brief 슬라이스 추가(브리핑 타입·API·AiSummaryPanel)
...
| `ops_enum_add` | `BookingStatus`에 `NO_SHOW`, `WaitlistStatus`에 `CONVERTED`·`EXPIRED` 추가 (no-show 루프 + waitlist 전환 추적) |
| `waitlist_offer_fields` | `Waitlist`에 `offeredSlotId`(공석 슬롯)·`offerExpiresAt`(TTL 만료) 추가 + `(status, offerExpiresAt)` 인덱스 |
...
| `20260612064400_consultation_prep_automation` | **상담 준비·생산성 자동화** — `Booking.concern String?`(고객 사전질문, write-only) + `Booking.briefOpenedAt DateTime?`(브리핑 최초 열람 시각) 추가. `ConsultationAiSummary`(`id, recordId @unique, status AiSummaryStatus @default(FALLBACK), model String?, content, createdAt, updatedAt`) 신규 모델 + `AiSummaryStatus { FALLBACK UPGRADED }` enum + `ConsultationRecord.aiSummary?` back-relation 추가. 순수 additive — 기존 테이블 ALTER 없음. |
