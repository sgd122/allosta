...
4. **관리자**로 로그인 → `/dashboard` → 전환율·outcome 분포·상품별 관심·**지표별 전환**(영문 `metricKey`를 `공복혈당` 등 한글 라벨로 표시)에 더해 **운영 퍼널 카드**(booked/confirmed/completed/noShow/cancelled), **운영 지표 카드**(noShowRate·slotUtilization·waitlistConversionRate), **챌린지 카드**(등록 수 + 등록 전환율 — null=구매 0건이면 "—", 0이면 "0%"), **브리핑 생산성 카드**(briefOpenRate)가 실시간 집계로 표시.
...
│   │   ├── ops-scheduler/          # @Interval 타이밍 래퍼 (sweepNoShows / sweepWaitlistOffers / sweepPendingUpgrades)
