...
| 4 | **시스템 설계 문서** | [`docs/04-system-design.md`](docs/04-system-design.md) (ERD·아키텍처·시퀀스·API·동시성) + [`docs/05-adr/`](docs/05-adr/) ADR 13종 |
...
NestJS (:3000) ──Prisma──▶ PostgreSQL (:5432, docker)
  ...
  ├── Analytics (scope 토글: own/all + 운영 퍼널/지표 + 챌린지 등록 수/전환율)
  ├── Waitlist (FIFO 공석 알림 + TTL advisory 오퍼 → CONVERTED/EXPIRED 전환 추적)
  ├── OpsScheduler (@Interval 타이밍 래퍼 — sweepNoShows / sweepWaitlistOffers)
...
4. **관리자**로 로그인 → `/dashboard` → 전환율·outcome 분포·상품별 관심·**지표별 전환**(영문 `metricKey`를 `공복혈당` 등 한글 라벨로 표시)에 더해 **운영 퍼널 카드**(booked/confirmed/completed/noShow/cancelled), **운영 지표 카드**(noShowRate·slotUtilization·waitlistConversionRate), **챌린지 카드**(등록 수 + 등록 전환율 — null=구매 0건이면 "—", 0이면 "0%")가 실시간 집계로 표시.
...
백엔드 통합 테스트 **15개 파일 / 15 suites / 104 tests** (`*.spec.ts` 14개 + 골든패스 `golden-path.e2e-spec.ts`).
챌린지·지표·cascade-cleanup 등 BioCom finalization 테스트(`challenge-enrollment.spec.ts`,
`analytics-challenge.spec.ts`, `test-result-metrics.spec.ts`, `cascade-cleanup.spec.ts`)도 동일 스위트에 포함되며,
어떤 assertion도 전체 스위트 개수를 하드코딩하지 않습니다(green/red로 회귀 판정). 프론트엔드 vitest: **14 files / 75 tests**(세션 JWT 서명 검증 `shared/auth/verify.test.ts`·라우트 접근 정책 `shared/auth/access.test.ts`·상담사 콘솔 일정 필터·그룹핑 순수 함수 `shared/lib/date`·`views/schedule/lib/filter`·`views/availability/lib/grouping` 포함). **합계 179 tests.**
...
│   │   ├── analytics/              # 전환 집계, scope 토글, 운영 퍼널/지표, 챌린지 등록 수/전환율
...
└── frontend/                       # Next.js 14 App Router · FSD(ADR 0009, types/constants 세그먼트 0012) · Tailwind on Radix + Jotai(ADR 0011)
    ...
    └── src/
        ...
        ├── entities/               # 12개 도메인 슬라이스 (types · constants · api: fetcher+queries.ts 훅 · ui) — 타입/상수 세그먼트 분리(ADR 0012)
